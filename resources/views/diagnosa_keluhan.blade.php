...
<body>
	...
	<section class="container mt-5" id="keluhan" style="height: 80vh;padding-top: 50px;">
		<div class="p-2">
			<h4 class="d-flex justify-content-center mb-5">Pilih Keluhan</h4>
			<div id="detailKeluhan" class="row" >
				<form style="margin: 0 auto;" method="post" action="{{ url('diagnosa/gejala') }}">
					<?= csrf_field() ?>
					<ul class="list-group " >
					  <?php foreach($keluhan as $row) { ?>
					  	<div>
					  		<li class="list-group-item noBorder">
					  			<input class="" type="checkbox" name="keluhan[]" id="<?= $row['kode_keluhan']; ?>" value="<?= $row['kode_keluhan']; ?>" style="cursor: pointer;">
					  			<label class="ml-2 " for="<?= $row['kode_keluhan']; ?>" style="cursor: pointer;"><?= $row['nama_keluhan']; ?></label>
					  		</li>
					  	</div>
					  <?php } ?>
					</ul>
